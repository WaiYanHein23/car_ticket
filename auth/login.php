...
if(isset($_POST['submit'])){
  ...
  if($email===''){
    $validate=false;
    $email_err="Email must not be bank";
  }

  if($password===''){
    $validate=false;
    $password_err="Password must not be bank";
  }
  ...
  if($validate){
    ...
    //$users = ["user_id"=>$user['user_id'], "user_name"=>$user['user_name'], "email"=>$user['email'],"password"=> $user['password'],"ph_no"=>$user['ph_no'],"address"=>$user['address']];
    $math=password_verify($password,$user['password']);
      if($math){
        $success=true;
        ...
        $invalid=true;
        $password_err_mess="Passsword does not match";
